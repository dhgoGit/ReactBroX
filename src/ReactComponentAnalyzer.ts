...
export class ReactComponentAnalyzer {
    ...
    private extractHookValue(path: any): string | undefined {
        ...
        // useState
        if (hookName === 'useState' && parent.type === 'VariableDeclarator' && 
            parent.id.type === 'ArrayPattern' && parent.id.elements.length >= 1) {
            const stateElement = parent.id.elements[0];
            if (stateElement && stateElement.type === 'Identifier') {
        ...
        // useReducer
        if (hookName === 'useReducer' && parent.type === 'VariableDeclarator' && 
            parent.id.type === 'ArrayPattern' && parent.id.elements.length >= 1) {
            const stateElement = parent.id.elements[0];
            if (stateElement && stateElement.type === 'Identifier') {
                return stateElement.name;
        ...
        // useContext
        if (hookName === 'useContext' && parent.type === 'VariableDeclarator' && 
            parent.id.type === 'Identifier') {
            return parent.id.name;
        }
        
        // Other hooks like useMemo, useCallback
    ...
    private extractContextValue(path: any): string | undefined {
        ...
        // Extract context value from variable declaration
        if (parent.type === 'VariableDeclarator' && parent.id.type === 'Identifier') {
            return parent.id.name;
        }
        
        // Extract context value from destructuring assignment
        if (parent.type === 'VariableDeclarator' && parent.id.type === 'ObjectPattern') {
            const properties = parent.id.properties;
            const propNames = properties.map((prop: any) => {
    ...
    private detectStoreUsage(path: any, componentInfo: ComponentInfo): void {
        ...
        if (callee.type === 'Identifier' && callee.name === 'useSelector') {
            this.addStoreInfo(componentInfo, 'redux', 'selectors', this.getCalleeArgumentName(path));
        } else if (callee.type === 'Identifier' && callee.name === 'useDispatch') {
            this.addStoreInfo(componentInfo, 'redux', 'actions', 'dispatch');
        }
        ...
        // Zustand detection
            ...
            this.addStoreInfo(componentInfo, 'zustand', 'selectors', this.getCalleeArgumentName(path));
        }
